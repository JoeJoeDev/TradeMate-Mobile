...
function QuoteCard({ quote, onPress }: { quote: Quote; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity style={styles.quoteCard} onPress={onPress} activeOpacity={0.7}>
      <View style={styles.quoteHeader}>
        <View>
          <Text style={styles.quoteNumber}>#{quote.quote_number}</Text>
          <Text style={styles.customerName} numberOfLines={1}>
            {quote.customer?.customer_name || 'No customer'}
          </Text>
        </View>
      ...
      <View style={styles.quoteDetails}>
        ...
        <View style={styles.detailRow}>
          ...
          <Text style={styles.detailValue}>{formatDate(quote.due_date)}</Text>
        </View>
      </View>

      <View style={styles.quoteFooter}>
        <Text style={styles.totalLabel}>Total</Text>
        <Text style={styles.totalAmount}>{formatCurrency(quote.total_amount)}</Text>
      </View>
    </TouchableOpacity>
  );
}
...
export default function QuotesScreen() {
  ...
  const { data, isLoading, refetch, isRefetching } = useQuotes({
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtitle}>
                {search ? 'Try a different search' : 'Create a quote to get started'}
              </Text>
            </View>
          }
        />
...
const styles = StyleSheet.create({
  ...
  quoteCard: { backgroundColor: '#ffffff', borderRadius: 12, padding: 16, marginBottom: 12 },
  quoteHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 },
  quoteNumber: { fontSize: 16, fontWeight: '700' },
  customerName: { fontSize: 14, color: '#6b7280', marginTop: 2 },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6 },
  statusText: { fontSize: 12, fontWeight: '600', textTransform: 'capitalize' },
  quoteDetails: { marginBottom: 12, gap: 4 },
